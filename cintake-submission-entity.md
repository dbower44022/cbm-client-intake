...
| `submitterEmail` | **varchar** | The submitter's email, for scanning / dedupe. **Must be `varchar`, NOT EspoCRM's `email` type.** EspoCRM's `email` type binds to the entity's single primary `emailAddress` field, so a custom-named email-type field silently stores nothing (verified live: the value stayed null whether the app sent a plain string or a `submitterEmailData` array). A plain `varchar` stores the string normally — as the sister `CInformationRequest.submitterEmail` (varchar) already does. **If this was created as type `email`, change it to `varchar` in the Entity Manager (and rebuild).** |
