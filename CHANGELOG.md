...
need a restart.** Doug's ruling: *"It is unacceptable to have settings hidden in
places that cannot be easily viewed and edited, so the rule going forward is that
all settings should be in the settings page."*

Ten settings were previously hidden from `/setup` — `analytics_enabled`,
`events_enabled`, `events_public_api`, `assignments_enabled`, `log_level`, the
three `intake_*` limits, `crm_config_refresh_seconds` and `release_tag`. They now
appear in a new **Restart required** group that explains what it is, and each row
shows the value **in force** beside the stored one, badged *Waiting for restart*
when they differ. The group heading carries a summary listing exactly which
changes are pending.

**This reverses the v0.190.1 lesson rather than forgetting it.** That failure was
a setting that *silently did nothing*: offered with a "takes effect on next
deploy" badge while `create_app` mounted routers from the environment and the
override layer loaded afterwards — so the override never applied at all, not even
after a redeploy, and toggling `events_enabled` produced a portal tile whose
routes did not exist. Hiding them was the cheap fix. Two changes make showing
them the honest one:

- **New `core/boot_overrides.py`.** `load_at_boot` installs the override layer at
  the very top of `create_app`, before routers are mounted, middleware built or
  logging configured — so "takes effect on restart" is now true rather than a
  promise. It **never raises** (a failure here would stop the app booting) and on
  a database failure it degrades to the **deployment's own values, never the code
  defaults**, which is settled ruling 6 of the System Settings design. It also
  detects a running event loop and skips rather than deadlocking.
- **The page reports what the process is really running.** For these keys the
  live `Settings` object is not the truth: the periodic refresh installs a newer
  value into it while the already-built routers carry on with the old one, so
  reading it back would report a change as taken effect when it had not.
  `boot_overrides` snapshots the real boot values and that snapshot is what the
  row reports as `inForce`. The page also says when the boot-time load itself
  *failed*, because then none of the stored settings are in force whatever the
  rows say.

`DENYLIST` now holds only secrets, infrastructure and the break-glass pair —
plus **`release_tag`**, which is curated **read-only** rather than hidden: it is
stamped into the container image, so a stored override would survive a restart
and make the deployment misreport which image it is running. Visible and
explained beats absent; `SettingSpec` gains a `readonly` flag for that case, and
the import-time sanity check permits curated-and-denylisted only when it is set.

Also: the save path's blocking `window.alert` is replaced with an inline,
dismissible banner. A modal interrupted the person to repeat what the row now
says beside the value.

24 new tests (`tests/test_boot_overrides.py`, plus rewrites in
`tests/test_system_settings.py` and `tests/test_version_stamps.py` that assert
the new rule and say why it replaced the old one). Full suite green. Verified in
a stub harness against a payload generated by the real service: the group, both
explanatory notes, the per-row "Running X now — will become Y when the app
restarts" line, the pending summary, the read-only row rendering no control, and
