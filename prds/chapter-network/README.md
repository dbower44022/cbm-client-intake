...
| **[1 — CRM config as an artifact](phase-1-crm-config.md)** | **In progress.** The interface contract is written; the conformance check is built (`ac6f1b4`) and run against both live CRMs. Stamp B's home is ruled (D1, `CNetworkStandard`), built on crm-test, and **both stamps are live at `/healthz` on every environment** (v0.214.0, 2026-08-28) — inert until the overlay switches are applied. `scripts/capture_roles.py` is written. **Criterion 13 — the throwaway round trip — is done** (2026-08-31), and it settled that roles can be written through the API, found that the file half is two trees, and produced the first live exit 4. | R7 (the extensions ruling); the two `doctl` applies (§ R6 — `CNetworkStandard` is now on BOTH CRMs, R0 closed 2026-08-31); prod's roles capture (§ R4); then the PRE_DEPLOY gate (§ R2) |
