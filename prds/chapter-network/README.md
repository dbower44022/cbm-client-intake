...
| **[1 — CRM config as an artifact](phase-1-crm-config.md)** | **In progress.** The interface contract is written; the conformance check is built (`ac6f1b4`) and run against both live CRMs. Stamp B's home is ruled (D1, `CNetworkStandard`), built on BOTH CRMs (crm-test 2026-08-27, production 2026-08-31 via `build_networkstandard.py`), and **both stamps are live and SWITCHED ON** (2026-08-31): crm-test and production report `releaseTag v0.217.0` and `crmConfig unstamped`. `scripts/capture_roles.py` is written. **Criterion 13 — the throwaway round trip — is done** (2026-08-31), and it settled that roles can be written through the API, found that the file half is two trees, and produced the first live exit 4. | R7 (the extensions ruling); prod's roles capture (§ R4); then the PRE_DEPLOY gate (§ R2). R0 and R6 closed 2026-08-31 |
| **[2 — Release train](phase-2-release-train.md)** | **Started.** Cadence ruled weekly, Sunday 17:00 UTC (2026-08-26); the CRM procedure is written; `scripts/cut_release.sh` exists and **two tags are cut** (`v0.214.0`, `v0.216.1`). `deploy_on_push` is still on for Cleveland's three apps, deliberately; the Lakeside app is the first with it off. **Decided 2026-08-31:** chapter apps follow a `release` branch the weekly cut fast-forwards; images-by-tag deferred. | R10 — the `release` branch and a `promote` script; then the stamps switched on |
