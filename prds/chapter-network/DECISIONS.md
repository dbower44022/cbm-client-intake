...
| 2026-09-13 | **Lakeside carries no demo or training data set, and gets no demo documentation.** It exists to prove a chapter can be stood up and provisioned (the Phase 6 branch-A Google rehearsal), so its build record is the documentation it needs. `scripts/sandbox/seed_training_data.py` keeps its crm-test-only guard; crm-test stays the single training sandbox. Revisit only if a non-Cleveland instance has to be *shown* to someone. | Asked and ruled 2026-09-13. The build record is [rehearsal-2026-08-31/](rehearsal-2026-08-31/README.md); the instance holds 4 engagements of test residue against crm-test's 18 |
...
| 2026-09-23 | **A chapter may keep its DNS at its existing provider; CRMBuilder's wizard gains a manual DNS mode, built before the Boston install.** Amends the 2026-09-18 ruling that every chapter's DNS lives in Cloudflare: Cloudflare stays the automatic path. Boston's `bbmentors.org` is at Squarespace Domains with DNSSEC on, so a move would have cost two to three hours and a change to its live email and website. Chosen (option C) over moving the DNS or building the server by hand. The install waits for the CRMBuilder change, which goes through that repository's requirement-first process. | `prompts/crmbuilder-manual-dns-kickoff.md`; [TASKS.md](TASKS.md) § G1 |
| 2026-09-23 | **Chapters train with the existing generic training users.** Reverses the 2026-09-18 ruling that chapters get their own set of training accounts: adding a set of users for every chapter was judged too much work. A chapter's staff sign in as the six training users Cleveland's own trainers use (Joe Mentor, Kitty Cat, Mentor Admin, Partner Manager, Sally Sponsor, Mark Marketing). The passwords are not changed after each chapter. Accepted cost: chapters and Cleveland's trainers share the same passwords, anyone who ever trained keeps access to the test system (which holds only invented records and sends no real email), and there is no training user for the Analytics Admin Team. | [deployment-guide/4-Work-Owed-Before-Any-Chapter.md](deployment-guide/4-Work-Owed-Before-Any-Chapter.md) item 6; guide steps 16.1 and 16.6 |
| 2026-09-23 | **Boston runs the current EspoCRM release, and Cleveland moves up to it later.** Boston's CRM is installed at whatever release CRMBuilder's deploy installs (10.0.6 on 2026-08-31), not pinned to Cleveland's 9.3.4. Chosen over pinning Boston to 9.3.4, which may have needed a CRMBuilder change on install day; the applications have already run against 10.0.6 on the Lakeside rehearsal. Accepted cost: two major CRM versions in the network until Cleveland upgrades, against ruling 4, and a problem may reproduce on one version and not the other. | [TASKS.md](TASKS.md) § C1; [deployment-guide/4-Work-Owed-Before-Any-Chapter.md](deployment-guide/4-Work-Owed-Before-Any-Chapter.md) item 2 |
...
| 2026-09-25 | **A Chapter acronym setting replaces the word CBM in every UI string.** `ORGANIZATION_ABBREVIATION`, default CBM, read from the chapter form's `chapter.abbreviation`; substituted as the `{{abbr}}` token in pages and scripts and by `branding.abbr()` in server messages, emails and calendar text, so Boston reads "ask BBM staff" and "BBM Contacts". Chosen over neutral wording with no abbreviation ("chapter staff"), which needs no setting but reads flatter. The CRM's stored list values and its own field labels are not UI strings and are a separate decision (TASKS G1 item 26). Built the same day as v0.232.0. | `core/branding.py`; `chapter-neutral-wording-inventory.md`; CHANGELOG 0.232.0 |
| 2026-09-24 | **Boston's hand build is its test system; its production deployment comes from the updated CRMBuilder.** Boston continues under this guide as it stands, and the result is a test system for Boston to learn on. When Boston decides to go into production, CRMBuilder makes the production deployment under the 2026-09-24 decisions, and nothing from the hand build is reconciled into CRMBuilder after the fact. Chosen over holding Boston until CRMBuilder is rebuilt, which is how a network loses its first chapter. Accepted cost: Boston's production date depends on the updated CRMBuilder, and a records move from the test system at go-live. CRMBuilder decision DEC-1160. | README phase 6; [phase-6](phase-6-first-chapter.md) |
| 2026-09-24 | **The fleet console is CRMBuilder, and its shape is ruled: client → application → deployment.** Nine CRMBuilder decisions (DEC-1152 to DEC-1160, session SES-433) settle proposal 8 and the A1 boundary in full: the settings list is defined in CRMBuilder, every chapter CRM is published from the design (never copied from crm-test), CRMBuilder deploys the apps to App Platform, holds every secret with a handover file to the chapter's vault, runs person-only steps from a readiness list with checks, gives chapter people a web sign-in that shows only their own deployment, and lets each chapter choose its update policy. DEC-1155 supersedes CRMBuilder's DEC-922 (one engagement per network): an application is defined by one client and deployed by many. | README phases 1, 3, 5; `phase-1-crm-config.md` § The decision trigger (answered); the CRMBuilder kickoff named in README phase 5 |
